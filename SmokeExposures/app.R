...
ui <- dashboardPage(
    ...
    dashboardBody(tabItems(
        tabItem(tabName = "smokeDash",
                ...
                box(title = "Heavy, Medium, and Light Wildfire Smoke Coverage from Jan 29, 2018 to Aug 20, 2018", 
                    width = "100%",
                    collapsible = T,
                    plotOutput("TimePlot")),
                ...
                fluidRow(
                    box(
                        title = "Heavy Wildfire Smoke Days over California (selected time period)",
                        collapsible = TRUE,
                        collapsed = F,
                    ...
                    #     )
                    # ),
                box(title = "Person-Days of Exposure by Race and Age Vulnerabilities (for selected Time Period)",
                    plotOutput('VulnPlot')
                )
        ...
        tabItem(tabName = "back",
         ...
         # wild fire smoke product box
                ...
                HTML("<h4>The Wildfire Smoke data on this dashboard comes from the Nation Oceanic and Atmospheric Administration's Office of Satellite and Product Operations.  Smoke data is part of the Office's <a href='https://www.ospo.noaa.gov/Products/land/hms.html' target='_blank'>Hazard Mapping System (HMS) Fire and Smoke Product</a></h4>
...
############### PLOT ############################   
   ...
   output$VulnPlot <- renderPlot({
       ...
       plotData() %>% ggplot(aes(x=cohort, y= PersonDays/1000000, fill = SmokeDensity)) + geom_bar(stat="identity", position ="dodge") + facet_wrap(~ race , scales="free_y") + ylab("Million Person-Days") + scale_fill_manual(values = c("yellow","orange","red"))
